...
public class PublishObservationV2 {
	...
	public static boolean send(JSONObject message) throws org.apache.http.HttpException {
		...
		try {
			...
			String sensor_id = null, sensor_name = null, timestamp = null, value = null, nivel = null, unit = null,
					type = null, altitude = null, longitude = null, latitude = null, sensor_place_name = null,
					info = null;
			...
			jsonObject2.put("observation", message.get("sensor_id").toString() + "_"
					+ message.get("timestamp").toString() + "_" + message.get("property").toString());

			Common.SOS_request getObservation = Common.httpPost_JSON(jsonObject2);
			
			JSONArray obs = (JSONArray) getObservation.getRequest().get("observations"); 
			
			if (getObservation.getResult() != 200 || obs.size() == 0) {
				...
				// observation.put("observedProperty",
				// message.get("property").toString());
				...
				if (message.get("type").equals("numeric")) {
					observation.put("observedProperty", message.get("type").toString());
				...
				if (message.get("sensor_id") != null && message.get("timestamp") != null
						&& message.get("property") != null)
					identifier.put("value", message.get("sensor_id").toString() + "_"
							+ message.get("timestamp").toString() + "_" + message.get("property").toString());
				...
				/*
				 * if (insertObservation.getResult() == 200) System.out.println(
				 * "After Insert Observation"); else if
				 * (insertObservation.getRequest().containsKey("exceptions") ==
				 * true) { JSONArray a = (JSONArray)
				 * insertObservation.getRequest().get("exceptions");
				 ...
				 * System.out.println(j.get("locator").toString()); if
				 * (j.get("locator").toString().equals("observedProperty")) {
				 * System.out.println("observedProperty is invalid");
				 * RegisterSensorV2.send(message);
				 * PublishObservationV2.send(message); } } } else if
				 * (insertObservation.getRequest().containsKey("exceptions") ==
				 * true) { JSONArray a = (JSONArray)
				 * insertObservation.getRequest().get("exceptions");
				 * System.out.println("exceptions");
				 ...
				 * if (j.containsKey("locator")) {
				 * System.out.println(j.get("locator").toString()); if
				 * (j.get("locator").toString().equals("offering")) {
